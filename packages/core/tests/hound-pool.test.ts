...
describe("HoundPool", () => {
  ...
  describe("activate()", () => {
    ...
    it("handles pool exhaustion with drop (default)", () => {
      ...
      expect(
        results.filter((r) => r.error === HOUND_PRESSURE_ERRORS.POOL_EXHAUSTED)
          .length,
      ).toBe(2);
    });
  });
  ...
  describe("terminate()", () => {
    it("terminates specific evidence by signature", () => {
      ...
      const terminateResult = results.find(
        (r) => r.error === "forced_terminate",
      );
  ...
  describe("PoolExhaustedAction", () => {
    it("defer queues up to limit", () => {
      ...
      expect(
        results.filter(
          (r) => r.error === HOUND_PRESSURE_ERRORS.DEFER_QUEUE_FULL,
        ).length,
      ).toBe(1);
    });
    ...
    it("escalate emits error but continues", () => {
      ...
      expect(
        results.filter(
          (r) => r.error === HOUND_PRESSURE_ERRORS.POOL_EXHAUSTED_ESCALATED,
        ).length,
      ).toBe(1);
      expect(pool.stats.totalErrors).toBeGreaterThan(0);
    });
  ...
  describe("Process error handling", () => {
    it("handles IPC decode errors", () => {
      ...
      expect(
        errorResults.some(
          (result) => result.error?.includes("spawn_failed") ?? false,
        ),
      ).toBe(false);
      expect(pool.stats.totalErrors).toBeGreaterThan(0);
    });
