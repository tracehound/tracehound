...
} from "../src/core/operational-events.js";
...
async function flushMicrotasks(): Promise<void> {
  ...
  if (
    typeof vitestTimers.isFakeTimers === "function" &&
    vitestTimers.isFakeTimers()
  ) {
...
describe("Tracehound Factory", () => {
  ...
  describe("Internal Wiring", () => {
    ...
    it("notifications fire on threat and quarantine", () => {
      ...
      expect(
        tracehound.notifications.stats.totalEmitted,
    ...
    it("houndPool error flow emits system_overload alert and panic", () => {
      ...
      expect(
        panics.some((panic) =>
          panic.reason.startsWith(SYSTEM_PANIC_REASONS.HOUND_ERROR_PREFIX),
        ),
      ).toBe(true);
      tracehound.shutdown();
    });
    ...
    it("sets watcher overload when pool is exhausted and clears it after recovery", async () => {
      ...
      expect(
        panics.some(
          (panic) =>
            panic.reason ===
            formatHoundErrorReason(HOUND_PRESSURE_ERRORS.POOL_EXHAUSTED),
        ),
      ).toBe(true);
      ...
      adapter.simulateMessage(
        pid,
        new Uint8Array(analysisMessage.subarray(4)).buffer,
      );
      ...
      adapter.simulateMessage(
        pid,
        new Uint8Array(completeMessage.subarray(4)).buffer,
      );
    ...
    it("keeps overload until deferred queue drains under defer pressure", async () => {
      ...
      expect(
        panics.some(
          (panic) =>
            panic.reason ===
            formatHoundErrorReason(HOUND_PRESSURE_ERRORS.DEFER_QUEUE_FULL),
        ),
      ).toBe(true);
      expect(tracehound.watcher.snapshot().overloaded).toBe(true);
      ...
      expect(
        panics.some(
          (panic) =>
            panic.reason ===
            formatHoundErrorReason(
              HOUND_PRESSURE_ERRORS.POOL_EXHAUSTED_ESCALATED,
            ),
        ),
      ).toBe(true);
      expect(tracehound.watcher.snapshot().overloaded).toBe(true);
  ...
  describe("Snapshot API", () => {
    ...
    it("uses default snapshot interval when intervalMs is omitted", async () => {
      ...
      const dir = mkdtempSync(
        join(tmpdir(), "tracehound-snapshot-default-interval-"),
      );
      const path = join(dir, "snapshot.json");
    ...
    it("emits snapshot_cleanup_failed panic when cleanup cannot unlink snapshot target", () => {
      const dir = mkdtempSync(
        join(tmpdir(), "tracehound-snapshot-cleanup-fail-"),
      );
      ...
      try {
        ...
        expect(
          panics.some(
            (panic) =>
              panic.reason === SYSTEM_PANIC_REASONS.SNAPSHOT_CLEANUP_FAILED,
          ),
        ).toBe(true);
      } finally {
        rmSync(dir, { recursive: true, force: true });
    ...
    it("emits snapshot_write_failed panic when snapshot flush fails", async () => {
      ...
      try {
        ...
        expect(
          panics.some(
            (panic) =>
              panic.reason === SYSTEM_PANIC_REASONS.SNAPSHOT_WRITE_FAILED,
          ),
        ).toBe(true);
