...
describe("tracehoundPlugin", () => {
  ...
  it("should return signature when emitSignatureInResponse is true", async () => {
    ...
    expect(reply.send).toHaveBeenCalledWith(
      expect.objectContaining({ signature: "test-sig" }),
    );
  });
    ...
    it("captures rawBody bytes into ingressBytes when available", () => {
      ...
      const req = createMockReq({
        rawBody: Buffer.from(
          '{"raw":true}',
          "utf8",
        ) as unknown as FastifyRequest["body"],
      ...
      expect(Buffer.from(scent.ingressBytes).toString("utf8")).toBe(
        '{"raw":true}',
      );
    });
