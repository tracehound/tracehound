...
import type { EvidenceSourceMetadata } from '../src/types/evidence.js'
import { hashBuffer } from '../src/utils/hash.js'

const defaultSource: EvidenceSourceMetadata = {
  ip: '127.0.0.1',
}
...
describe('Evidence', () => {
  ...
  describe('construction', () => {
    it('accepts valid inputs', () => {
      const evidence = new Evidence(validBytes, validSignature, validHash, 'high', Date.now(), defaultSource)
  ...
  describe('getters', () => {
    ...
    it('exposes readonly properties', () => {
      ...
      const evidence = new Evidence(validBytes, validSignature, validHash, 'critical', captured, defaultSource)
    ...
    it('exposes whether evidence bytes were stored in compressed form', () => {
      const evidence = new Evidence(validBytes, validSignature, validHash, 'high', Date.now(), defaultSource, true)

      expect(evidence.compressed).toBe(true)
    })

    it('exposes source metadata', () => {
      const source: EvidenceSourceMetadata = {
        ip: '192.168.1.1',
        userAgent: 'test-agent',
        tls: { cipherSuite: 'TLS_AES_256_GCM_SHA384', version: 'TLSv1.3' },
      }
      const evidence = new Evidence(validBytes, validSignature, validHash, 'high', Date.now(), source)
  ...
  describe('neutralize', () => {
    ...
    it('generates unique record IDs', () => {
      const e1 = new Evidence(validBytes, validSignature, validHash, 'high', Date.now(), defaultSource)
      ...
      const e2 = new Evidence(bytes2, validSignature, hashBuffer(bytes2), 'high', Date.now(), defaultSource)
  ...
  describe('evacuate', () => {
    ...
    it('uses the injected clock for evacuation timestamps', () => {
      const evidence = new Evidence(validBytes, validSignature, validHash, 'high', 1000, defaultSource, false, () => 4242)
      const record = evidence.evacuate('s3://bucket/path')
