...
export class SecurityState implements ISecurityState {
  ...
  recordThreat(category: string, severity: Severity): void {
    this._threatTotal++;
    this._threatsByCategory.set(
      category,
      (this._threatsByCategory.get(category) ?? 0) + 1,
    );
    this._threatsBySeverity.set(
      severity,
      (this._threatsBySeverity.get(severity) ?? 0) + 1,
    );
  ...
  get history(): readonly SecurityHistoryEntry[] {
    if (!this._historyFull)
      return this._history.slice(0, this._historyWriteIndex);
    return [
  ...
  get stats(): SecurityStateStats {
    return {
      ...
      oldestEntry:
        this._history.length > 0 ? this._history[0]!.timestamp : null,
      newestEntry:
  ...
  private addHistory(
    type: SecurityHistoryEntry["type"],
    data: Record<string, unknown>,
  ): void {
    ...
    this._historyWriteIndex =
      (this._historyWriteIndex + 1) % this.config.maxHistorySize;
  }
}
