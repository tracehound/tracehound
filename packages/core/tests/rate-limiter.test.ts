...
describe("RateLimiter", () => {
  ...
  describe("construction", () => {
    ...
    it("throws on non-positive windowMs", () => {
      expect(() => new RateLimiter({ ...config, windowMs: 0 })).toThrow(
  ...
  describe("check", () => {
    ...
    it("blocks same-IP user-agent rotation via IP ceiling", () => {
      ...
      for (let i = 0; i < config.maxRequests; i++) {
        expect(
          limiter.check(createSource("192.168.1.1", "Mozilla/5.0")).allowed,
        ).toBe(true);
      }
    ...
    it("does not create new composite entries when IP ceiling rejects rotated fingerprints", () => {
      ...
      for (let i = 0; i < config.maxRequests; i++) {
        expect(limiter.check(createSource(attackerIp, "base-ua")).allowed).toBe(
          true,
        );
      }
