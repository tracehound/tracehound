    ...
    for (const seed of manifest.seeds) {
      ...
      if (entry.type === "agent") {
        ...
        if (typeof entry.expect.quarantineCountDelta === "number") {
          expect(after.count - before.count).toBe(
            entry.expect.quarantineCountDelta,
          );
        }
      }
      ...
      if (entry.type === "ipc") {
        ...
        if (entry.expect.throws) {
          ...
          expect(parser.bufferedBytes).toBe(
            entry.expect.bufferedBytesAfterThrow,
          );
        } else {
      ...
      if (entry.type === "duplicate") {
        ...
        expect(runtime.quarantine.stats.count).toBe(
          entry.expect.quarantineCount,
        );
      }
      ...
      if (entry.type === "state-machine-permutation") {
        ...
        expect(
          runtimeA.quarantine.stats.count === runtimeB.quarantine.stats.count,
        ).toBe(entry.expect.sameConvergedCount);
      }
      ...
      if (entry.type === "state-machine-rollback") {
        ...
        if (entry.expect.stableCount) {
          expect(runtimeA.quarantine.stats.count).toBe(
            runtimeB.quarantine.stats.count,
          );
        }
        if (entry.expect.deterministicOutcome) {
          expect(runtimeA.quarantine.stats.bytes).toBe(
            runtimeB.quarantine.stats.bytes,
          );
        }
      }
      ...
      if (entry.type === "canonical-pair") {
        ...
        expect(left.canonical === right.canonical).toBe(
          entry.expectCanonicalEqual,
        );
      }
